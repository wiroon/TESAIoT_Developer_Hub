 ...
 * 0 = GPU-idle check mode: only flush when GPU is idle (never blocks with
 *     infinite timeout).  All pending commands are submitted at end-of-frame
 *     via lv_vg_lite_finish() which uses a safe 5 s timeout.
 * N>0 = batch N draw ops before flushing (uses stall(INFINITE) which can
 *       deadlock if GPU is slow). */
